...
class CascadeDetector:
    ...
    def detectFace(self, bgr_image): #bgr_image as input i.e. the frame
        # self.PUPIL_THRESH = cv2.getTrackbarPos('Eye Detection Threshold', 'EyePaint') # obtained from the slider on EyePaint Window
        gray_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2GRAY) # Converting the image to grey scale
        return self.face_cascade.detectMultiScale(gray_image, 1.3, 5)  # returns (x,y,w,h) x,y of top-left
    ...
    def find_eyes(self, frame):
        try:
            # pos = cv2.getTrackbarPos('Eye Detection Threshold', 'EyePaint')
            print("Trackbar position: 60")  # Ensure this line executes without errors
        except cv2.error as e:
            print("Error accessing trackbar:", e)
        # self.PUPIL_THRESH = cv2.getTrackbarPos('Eye Detection Threshold', 'EyePaint')
        # frame is each captured frame (image) from the video
        frame_w = frame.shape[1] # frame.shape_tuple = (frame_height, frame_width, number_of_channels)
        frame_h = frame.shape[0]
        frame_ratio = frame_w / frame_h
        frame_original = cv2.copyTo(frame, None) # creating a copy at dest. None for later comparisons

        faces = self.detectFace(frame) # calling the 1st function; faces = [(x1,y1,w1,h1), (x2,y2,w2,h2),....]
        for (x, y, w, h) in faces:
            ...
            for (ex, ey, ew, eh) in eyes:
                ...
                if ex + ew / 2 < face_w / 2:
                    ...
                    lp_keypoint, lt_img = self.detectPupils(self.left_eye_frame, 60)
                    ...
                    rp_keypoint, rt_img = self.detectPupils(self.right_eye_frame, 60)
